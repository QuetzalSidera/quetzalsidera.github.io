...
这是 QuetzalSidera 的个人博客项目，原由 VitePress + 自定义 Vue 主题搭建，目前正在迁移到 **Next.js App Router**（React + CSS Modules）。迁移初期目标参考 Nextra，但当前实现采用自定义 App Router 路由与 `next-mdx-remote/rsc`，以保持原有 `posts/` / `collections/` 内容结构。
...
- 目标框架：Next.js App Router + React + CSS Modules
...
- 部署方式：**Cloudflare Pages + Git Integration**（自定义域名 `quetzalsidera.me`）。Cloudflare 直接监听 GitHub 仓库的
  `main` 分支推送，构建命令、产物目录、环境变量均在 Cloudflare Pages 控制台配置；迁移后产物目录由 `.vitepress/dist` 切换为
  Next.js 静态导出目录（`out/`）。
...
| 维度   | 现状（VitePress）                                                            | 目标（Next.js）                                                   |
|------|--------------------------------------------------------------------------|---------------------------------------------------------------|
| 框架   | VitePress 1.x                                                            | Next.js App Router                                             |
...
| 路由   | 文件路由（`posts/*.md`）                                                       | App Router 动态路由（`app/posts/[slug]/page.tsx`）                  |
| 包管理器 | pnpm                                                                     | pnpm                                                          |
| 部署   | Cloudflare Pages Git Integration（构建命令 `pnpm build`，产物 `.vitepress/dist`） | Cloudflare Pages Git Integration（构建命令 `pnpm build`，产物 `out/`） |

## 主题配置

VitePress 主题主要配置文件位于 [`.vitepress/config.mts`](.vitepress/config.mts) 与 [
`.vitepress/theme/config/site.ts`](.vitepress/theme/config/site.ts)。
...
VitePress 旧版入口在迁移期仍保留为 `vp:*` 脚本，用于对照与回退。
...
- `.vitepress/`：旧 VitePress 站点配置与 Vue 主题代码（最终切换前保留作对照）
