...
const commitImage = {
  src: miscellaneousImagePath['01-commit'],
  alt: 'Github Commit',
  align: 'right',
  wrap: true,
  maxHeight: '16rem',
...
1. **代码仓库**在 GitHub
2. **域名DNS**在阿里云解析
3. **构建和部署**使用 GitHub Actions → GitHub Pages

现在变成了：

1. **代码仓库**仍在 GitHub（不需要迁移）
2. **DNS解析**迁移到 Cloudflare DNS
3. **构建**由 GitHub Actions 完成，**部署**通过 Github Action + Wrangler 直接推到 Cloudflare Pages
