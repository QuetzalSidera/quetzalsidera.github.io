...
在 Unix 里，目录通常也被视为文件，它的内容是一组“文件名 -> inode”的映射。
...
文件系统管理的直接对象通常是卷，整块磁盘会先被划分为一个或多个分区或卷。
...
不同文件系统命名不同，但概念相近。比如 Unix 中常说的 `superblock`，就是卷控制块，描述整个卷的元数据。
