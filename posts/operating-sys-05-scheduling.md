...
在 [进程基础](./operating-sys-01-process.md) 中已介绍进程状态、就绪队列、调度器和上下文切换；[线程基础](./operating-sys-03-thread.md) 补充了线程模型及用户线程与内核线程的区别。本篇将这些概念收拢到调度主题下：谁先运行、运行多久、什么时候切换，以及在多线程、多核和实时系统中如何变化。
...
| 保存主体 | 主要由硬件自动保存最基本部分 | 主要由内核软件保存到 PCB |
| 目的 | 保存原始状态，处理完返回原执行流 | 保存原进程信息，让新进程运行 |
| 生命周期 | 通常很短，仅服务于一次中断处理 | 与具体进程绑定 |
