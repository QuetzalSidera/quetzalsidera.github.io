...
实时系统的重点是“能否在需要的时候及时完成”，平均速度只是辅助指标。
...
- `policy` 只表示调度规则类别，完整调度配置还包括优先级等参数
...
因此，调度贯穿单核、多核、线程模型和实时约束，不只是算法清单。同步、锁和并发控制还会继续接上这条主线：什么时候阻塞、什么时候唤醒。
