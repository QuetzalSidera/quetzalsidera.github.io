...
outline:
  ...
  - title: exec 函数族
    slug: exec-函数族
    level: 2
  - title: 5. exit 与 _exit
    slug: _exit-与-exit-退出程序
    level: 1

  - title: fork + exec + wait 示例
    slug: 示例-fork-exec-wait

  - title: 小结
    slug: 小结
  - title: 1. Unix 权限
    slug: permission-note
    level: 1
  - title: 2. SIGCHLD
    slug: SIGCHLD
    level: 1
...
### 2. 接口关系{#POSIX-系统调用-ISO-C-C-标准库的关系}
...
### 3. read / write{#read-write-读写文件描述符}
...
## POSIX 进程 API{#posix-进程-API}

进程创建与程序执行经常伴随文件描述符的继承、重定向和关闭，因此基础 I/O 接口是理解进程 API 的前置知识。

### 1. getpid 与 getppid{#getpid-与-getppid}
...
### 3. wait 与 waitpid{#wait-与-waitpid等待子进程结束}
...
`status` 是一组经过编码的状态信息，不能按简单的“0 或 1”处理，通常要配合宏来解析：
...
[man 手册](https://www.man7.org/linux/man-pages/man3/exec.3.html)中有如下说明：
...
### 5. exit 与 _exit{#_exit-与-exit-退出程序}
...
## fork + exec + wait 示例{#示例-fork-exec-wait}
