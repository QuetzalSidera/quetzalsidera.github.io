<template>
  ...
  onMounted(() => {
      ...
      Object.keys(toggles).forEach((key) => {
          ...
          if (storedValue !== null) {
              state[key as keyof typeof toggles] = JSON.parse(storedValue);
          }
      });
  });

  onUnmounted(() => {
      if (darkModeMediaQuery && handleSystemThemeChange) {
  ...
  const toggleSwitch = (key: string) => {
      const typedKey = key as keyof typeof toggles
      const isChecked = state[typedKey];
      state[typedKey] = !isChecked;
