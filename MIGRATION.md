...
| giscus：评论区加载、登录入口、浅/深色自定义主题、HTTPS 主题 CSS 正常                  | [测试通过]                                                                                                  | [测试通过] 已修复待回归：giscus 主题 CSS 扩展到 iframe 内的 `html` / `body` / `main`，补齐未带 `main` 前缀的 Primer class 覆盖；主题变更时重建 widget，并更新主题 URL 版本参数。      | [测试通过]                        | [测试通过]                                |
