...
- [x] `package.json`：新增 `next@^15` / `react@^19` / `react-dom@^19` / `next-mdx-remote` /
  ...
  `vitepress` / `vue` / `less` / `markdown-it-*` 移到 devDependencies 并加 `vp:*` 前缀脚本保留 VitePress 入口
- [x] 拆分 `tsconfig.json`（给 Next.js 用）与 `tsconfig.vitepress.json`（给 vue-tsc 用）
...
- [x] `lib/mdx.ts` + `components/BlogImage.tsx`：VitePress Markdown 兼容层（移除 Vue `<script setup>`、兼容旧标题锚点 /
  `Image v-bind` / TeX 旋量符号 / 裸小写 HTML 标签）
...
- [ ] Cloudflare preview 与视觉回归通过后，删除 `.vitepress/`，整理 `package.json` 依赖（清掉 vitepress / vue / less /
...
13. **`.vitepress/` 保留边界**：Next 运行时代码已不再 import `.vitepress/`；该目录只保留为迁移对照和回退材料，等 Cloudflare
    preview、视觉回归与测试方案通过后再删除。
...
测试基准：Cloudflare Pages `react-migration` preview 链接为被测对象，`main` 分支线上 VitePress
版本作为视觉与行为对照。除非特别说明，每个测试项都需要覆盖浅色 / 深色 / 系统主题三态。
...
| Navbar：居中稳定、移动/缩放/刷新不横向漂移，菜单与控件可用                            | [测试通过]                                                                                                  | [测试通过]                                                                                                                                     | [测试通过]                        | [未测试]                                 |
| 主题切换：浅色 / 深色 / 系统三态无 hydration mismatch，背景与遮罩同步              | [测试通过]                                                                                                  | [测试通过]                                                                                                                                     | [测试通过]                        | [未测试]                                 |
| WelcomeBox：文字、motto、社交图标、3D hover 效果正常                       | [测试通过]                                                                                                  | [测试通过]                                                                                                                                     | [测试通过]                        | [未测试]                                 |
...
| giscus：评论区加载、登录入口、浅/深色自定义主题、HTTPS 主题 CSS 正常                  | [测试通过]                                                                                                  | [未测试] 已修复待回归：giscus 主题 CSS 扩展到 iframe 内的 `html` / `body` / `main`，补齐未带 `main` 前缀的 Primer class 覆盖；主题变更时重建 widget，并更新主题 URL 版本参数。      | [测试通过]                        | [测试通过]                                |
...
1. 在 Cloudflare Pages 控制台把 `react-migration` 加为 preview branch，使用构建命令 `pnpm build`、产物目录 `out/`
   验证预览部署。[已结束]
2. 用 preview 链接做 main/VitePress 与 react-migration/Next 的视觉并排回归，重点检查字体、动画、图片、KaTeX、PostSideList、WelcomeBox
   3D、搜索结果、文章正文样式、giscus 评论与打印 PDF。[ok]
3. 预览验证通过后，再删除 `.vitepress/` 并清理 VitePress/Vue/Less/markdown-it/wrangler 等仅旧链路使用的依赖与脚本。
4. 将 Cloudflare Pages 生产构建目录从 `.vitepress/dist` 切到 `out/`，再做 Lighthouse / Pagespeed 检查并 PR
   `react-migration` → `main`。
